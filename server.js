...
function freshRoom(code,buyin){
  return { code, buyin:(buyin==null?1000:buyin), pote:0, practice:false,
...
function armTimer(code){
  ...
  r.timer=setTimeout(()=>{
    ...
    const ss=rr.g.turn, mv=botPick(rr.g,ss);
...
io.on('connection',(socket)=>{
  ...
  socket.on('adminUsers',({key},cb)=>{
    ...
    const list=Object.values(accounts).map(a=>({user:a.user,wallet:a.wallet,net:a.net,games:a.games,wins:a.wins,
      email:a.email||'',phone:a.phone||'',verified:a.emailVerified!==false})).sort((x,y)=>y.wallet-x.wallet);
    cb&&cb({ok:true, list});
  });
